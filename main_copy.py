...
class Train_Dataset(Dataset):
    def __init__(self, file_ng, file_ok, transforms_=None):
        self.transform = transforms_
        self.file_ng = file_ng
        self.file_ok = file_ok        

    def __getitem__(self, index):
        img_ng = Image.open(self.file_ng[index % len(self.file_ng)])
        img_ok = Image.open(self.file_ok[index % len(self.file_ok)])

        img_ng = self.transform(img_ng)
        img_ok = self.transform(img_ok)

        return {"ng": img_ng, "ok": img_ok}

    def __len__(self):
        return len(self.file_ok)    

class Test_Dataset(Dataset):
    def __init__(self, file, transforms_=None):
        self.transform = transforms_
        self.file = file      

    def __getitem__(self, index):
        img = Image.open(self.file[index % len(self.file)])
...
# ok_path     = r'./datasets/train/ok'
# ng_path     = r'./datasets/train/ng'
# test_path   = r'./datasets/test/ok'

ok_path     = r'./datasets/capsule/train_ok'
ng_path     = r'./datasets/capsule/train_ng'
test_path   = r'./datasets/capsule/test_ok'
save_dir    = r'./result'
...
files_ok    = [os.path.join(ok_path, f) for f in os.listdir(ok_path) if f.endswith('.png')]
files_ng    = [os.path.join(ng_path, f) for f in os.listdir(ng_path) if f.endswith('.png')]
files_test  = [os.path.join(test_path, f) for f in os.listdir(test_path) if f.endswith('.png')]
train_batch_size    = 8
test_batch_size     = 8
ch = 3

train_dataset       = Train_Dataset(file_ng=files_ng, file_ok=files_ok, transforms_=transforms_)
val_dataset         = Test_Dataset(files_test, transforms_=transforms_)

train_dataloader    = DataLoader(train_dataset, batch_size=train_batch_size, shuffle=True, num_workers=4)
val_dataloader      = DataLoader(val_dataset, batch_size=test_batch_size, shuffle=True, num_workers=4)
...
for epoch in range(n_epochs):
    for i, batch in enumerate(train_dataloader):
        ...
        real_ng = batch["ng"].cuda()
        real_ok = batch["ok"].cuda()

        # 진짜(real) 이미지와 가짜(fake) 이미지에 대한 정답 레이블 생성 (너바와 높이를 16씩 나눈 크기)
        real = torch.cuda.FloatTensor(real_ng.size(0), 1, 16, 16).fill_(1.0) # 진짜(real): 1
        fake = torch.cuda.FloatTensor(real_ng.size(0), 1, 16, 16).fill_(0.0) # 가짜(fake): 0

        """ 생성자(generator)를 학습합니다. """
        optimizer_G.zero_grad()

        # 이미지 생성
        gen_ng = generator(real_ok)
        

        # 생성자(generator)의 손실(loss) 값 계산
        loss_GAN = criterion_GAN(discriminator(gen_ng, real_ok), real)

        # 픽셀 단위(pixel-wise) L1 손실 값 계산
        loss_pixel = criterion_pixelwise(gen_ng, real_ng) 
        ...
        loss_real = criterion_GAN(discriminator(real_ng, real_ok), real) # 조건(condition): real_A
        loss_fake = criterion_GAN(discriminator(gen_ng.detach(), real_ok), fake)
        ...
        if done % sample_interval == 0:
            ...
            train_sample = torch.cat((real_ok.data, real_ng.data, gen_ng.data), -2)
            save_image(train_sample, os.path.join(save_dir, f"train_{done}.png"))
            
            test_sample = torch.cat((real_A.data, fake_B.data), -2)
