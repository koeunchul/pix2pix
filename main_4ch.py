...
class Train_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return {"ng": img_ng, "ok": img_ok, "mask": img_mask, "in": img_in}
...
class Test_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return {"ok": img_ok, "mask": img_mask, "in": img_in}
...
discriminator   = Discriminator(in_channels=dis_ch)
...
for epoch in range(n_epochs):
    for i, batch in enumerate(train_dataloader):
        ...
        # 이미지 생성
        train_gen_6ch = generator(train_in)

        
        # 3채널로 분리
        train_gen, train_gen_mask = torch.split(train_gen_6ch, split_size_or_sections=3, dim=1)        
        ...
        if done % sample_interval == 0:
            ...
            test_gen_6ch    = generator(test_in)
            # 3채널로 분리
