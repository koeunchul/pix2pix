...
class Train_Dataset(Dataset):
    def __init__(self, ng, ok, mask, label, transforms_=None):
    ...
    def __getitem__(self, index):
        ...
        # 6채널로 합치기 - 두 텐서를 채널 차원(dim=0)을 따라 연결
        img_in = torch.cat((img_ok, img_mask), dim=0)
        label = self.label[index % len(self.label)]

        return {"ng": img_ng, "ok": img_ok, "mask": img_mask, "in": img_in, "label":label}

    def __len__(self):
        return len(self.ok)    

class Test_Dataset(Dataset):
    def __init__(self, ok, mask, label, transforms_=None):
        self.transform = transforms_
        self.ok     = ok      
        self.mask   = mask      
        self.label  = label
    ...
    def __getitem__(self, index):
        ...
        # 두 텐서를 채널 차원(dim=0)을 따라 연결
        img_in = torch.cat((img_ok, img_mask), dim=0)
        label = self.label[index % len(self.label)]
        # print(img_in.shape)

        return {"ok": img_ok, "mask": img_mask, "in": img_in, "label": label}
...
def get_label_from_filename(filename):
    if 'crack' in filename:
        return 0
    elif 'faultyimprint' in filename:
        return 1
    elif 'poke' in filename:
        return 2
    elif 'scratch' in filename:
        return 3
    elif 'squeeze' in filename:
        return 4
    else:
        return None  # 혹은 다른 기본값이 필요할 경우 그 값을 지정해주면 됩니다.

def generate_random_labels(length, classnum):
    return [random.randint(0, classnum) for _ in range(length)]
...
train_labels    = [get_label_from_filename(os.path.basename(f)) for f in train_ok_path]
test_labels     = generate_random_labels(len(test_mask_path), classnum=classnum)

train_dataset       = Train_Dataset(ng=train_ng_path, ok=train_ok_path, mask=train_mask_path, label=train_labels, transforms_=transforms_)
test_dataset        = Test_Dataset(ok=test_ok_path, mask=test_mask_path, label=test_labels, transforms_=transforms_)
...
class GeneratorUNet(nn.Module):
    ...
    def forward(self, x, labels):
        ...
        # 레이블 정보 확장
        labels = labels.view(labels.size(0), 1, 1, 1).expand(-1, 1, x.size(2), x.size(3))
...
class Discriminator(nn.Module):
    def __init__(self, in_channels=3):
        ...
        self.model = nn.Sequential(
            ...
            *discriminator_block(in_channels, 64, normalization=False), # 출력: [64 X 128 X 128]
    ...
    def forward(self, img_A, img_B, labels):
        ...
        # 레이블 정보 확장
        labels = labels.view(labels.size(0), 1, 1, 1).expand(-1, 1, img_input.size(2), img_input.size(3))
        
        # 영상과 레이블 정보 연결
        img_input = torch.cat([img_input, labels], 1)
...
generator       = GeneratorUNet(in_channels=gen_ch, out_channels=gen_ch)
discriminator   = Discriminator(in_channels=gen_ch)
...
for epoch in range(n_epochs):
    for i, batch in enumerate(train_dataloader):
        ...
        train_in    = batch["in"].cuda()
        train_label = batch["label"].cuda()

        # 진짜(real) 이미지와 가짜(fake) 이미지에 대한 정답 레이블 생성 (너바와 높이를 16씩 나눈 크기)
        real = torch.cuda.FloatTensor(train_ok.size(0), 1, 16, 16).fill_(1.0) # 진짜(real): 1
        fake = torch.cuda.FloatTensor(train_ok.size(0), 1, 16, 16).fill_(0.0) # 가짜(fake): 0

        """ 생성자(generator)를 학습합니다. """
        optimizer_G.zero_grad()

        # 이미지 생성
        train_gen_7ch = generator(train_in, train_label)

        # gen, gen_mask, label로 채널 분리
        train_gen, train_gen_mask, gen_label = torch.split(train_gen_7ch, split_size_or_sections=[3, 3, 1], dim=1)

        # # 3채널로 분리
        # train_gen, train_gen_mask = torch.split(train_gen_7ch, split_size_or_sections=3, dim=1)        

        # 생성자(generator)의 손실(loss) 값 계산
        loss_GAN = criterion_GAN(discriminator(train_gen, train_ok, train_label), real)
        ...
        loss_real = criterion_GAN(discriminator(train_ng, train_ok, train_label), real) # 조건(condition): real_A
        loss_fake = criterion_GAN(discriminator(train_gen.detach(), train_ok, train_label), fake)
        ...
        if done % sample_interval == 0:
            ...
            test_mask       = imgs["mask"].cuda()
            test_in         = imgs["in"].cuda()
            test_label      = imgs["label"].cuda()
            test_gen_7ch    = generator(test_in, test_label)
            
            # gen, gen_mask, label로 채널 분리
            test_gen, test_gen_mask, test_label = torch.split(test_gen_7ch, split_size_or_sections=[3, 3, 1], dim=1)
            
            # # 3채널로 분리
            # test_gen, test_gen_mask = torch.split(test_gen_6ch, split_size_or_sections=3, dim=1)    
            
            train_sample = torch.cat((train_ok.data, train_mask.data, train_ng.data, train_gen.data), -2)
            save_image(train_sample, os.path.join(save_dir, f"train_{done}.png"))
            
            test_sample = torch.cat((test_ok.data, test_mask.data, test_gen.data), -2)
